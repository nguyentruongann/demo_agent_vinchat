...
function Register() {
  ...
  async function handleSubmit(event) {
    ...
    if (!email.trim() && !phone.trim()) {
      setError(t.registerContactRequired)
      return
    }
    setSubmitting(true)
    try {
    ...
    <main className="register-page">
      <section className="register-page__card">
        ...
        <form className="register-page__form" onSubmit={handleSubmit}>
          ...
          <div className="register-page__field">
            <label className="register-page__label" htmlFor="register-phone"><Phone className="register-page__label-icon" /><span>{t.phone}</span></label>
          ...
          <div className="register-page__field">
            ...
            <input className="register-page__input" id="register-password" type="password" required minLength="8" autoComplete="new-password" placeholder={t.passwordMinimum} value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>
          {error && <p className="register-page__error" role="alert">{error}</p>}
          <button className="register-page__submit" type="submit" disabled={submitting}><UserPlus className="register-page__submit-icon" /><span>{submitting ? t.creatingAccount : t.createAccount}</span></button>
        </form>
