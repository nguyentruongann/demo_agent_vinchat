...
class RAGService:
    ...
    def __init__(self) -> None:
        ...
        self.collection = self.chroma.get_or_create_collection(
            ...
            metadata=self._collection_contract(),
        )
        if self.collection.count() > 0:
            actual = dict(self.collection.metadata or {})
            mismatches = {
                key: (actual.get(key), expected)
                for key, expected in self._collection_contract().items()
                if actual.get(key) != expected
            }
            if mismatches:
                raise RuntimeError(
                    "Chroma collection embedding contract mismatch; rebuild the configured "
                    f"collection. mismatches={mismatches}"
                )
        # FAQ-first is separate from generic retrieval but reads active PostgreSQL rows.
        self.faq_matcher = FAQMatcher(
            embed_passages=self.embed_documents,
            embed_queries=self.embed_queries,
            rows_loader=load_faq_corpus,
            semantic_verifier=self._verify_faq_candidates,
        )

    def _collection_contract(self) -> dict[str, str | int]:
        return {
            "hnsw:space": "cosine",
            "embedding_backend": "gemini_api",
            "embedding_model": self.settings.gemini_embedding_model,
            "embedding_dimension": int(self.settings.embedding_dimension),
            "knowledge_schema_version": int(self.settings.knowledge_schema_version),
        }
    ...
    def embed_query(self, query: str) -> list[float]:
        return self.model.encode_query([query])[0].tolist()

    def embed_queries(self, queries: list[str]) -> np.ndarray:
        """Embed several query strings in one Gemini batch.
    ...
    def semantic_search_many(
        ...
        """Run multiple semantic queries in one Gemini batch and one Chroma call.
    ...
    @staticmethod
    def _metadata_identifier_values(value: Any) -> set[str]:
        """Return comparable scalar IDs from Chroma's serialized PK metadata."""
    ...
    def _named_entity_scope(
        self,
        entities: list[dict[str, Any]],
    ) -> dict[str, list[str]]:
        """Build a closed parent/entity scope for exact follow-up retrieval.

        A property row is stored with ``entity_id=id=<property>`` while its room and
        dining children carry ``property_id=<property>``.  Preserving both forms lets
        an exhaustive request mean "all rooms of this resort" instead of "all hotel
        ...
        for entity in entities or []:
            ...
            for index in entity.get("indices") or []:
                ...
                for field in (
                    ...
                    entity_ids.update(
                        self._metadata_identifier_values(metadata.get(field))
                    )
    ...
    @classmethod
    def _document_matches_named_entity_scope(
        cls,
        item: dict[str, Any],
        scope: dict[str, list[str]],
    ) -> bool:
        """Accept the named entity itself or a structurally linked child row."""
        metadata = item.get("metadata", {}) or {}
        normalized_names = {
        ...
        entity_name = normalize_text(str(metadata.get("entity_name") or ""))
        if entity_name and entity_name in normalized_names:
            return True
        ...
        for field in (
            ...
            if cls._metadata_identifier_values(metadata.get(field)) & entity_ids:
                return True
        return False

    @staticmethod
    def _is_room_catalog_query(*texts: str) -> bool:
        combined = normalize_text(" ".join(str(value or "") for value in texts))
        markers = (
            "loai phong", "hang phong", "nhung phong nao", "cac phong nao",
            "room type", "types of room", "rooms available", "available room",
            "room options", "room categories",
        )
    ...
    def _rerank_candidates(
        ...
        # in process memory. Gemini is used for the query vector.
    ...
    def hybrid_search(
        ...
        resolved_entity_names: list[str] | None = None,
        ...
        named_entities = self._find_named_entity_mentions(
            user_message,
            query,
            *(resolved_entity_names or []),
        )
        ...
        named_entity_scope = self._named_entity_scope(named_entities) if named_entities else {
            "names": [],
            "normalized_names": [],
            "entity_ids": [],
            "entity_types": [],
            ...
            for raw_intent in retrieval_intents:
                ...
                for destination in destinations:
                    ...
                    if (
                        ...
                        and intent == "hotel"
                        and self._is_room_catalog_query(user_message, query, focused_query)
                    ):
                        # "All room types at this resort" is a child-record request,
                        # not a request for every peer property at the destination.
                        effective_types = {"room"}
                    candidates = self.keyword_candidates(
                        destination=destination,
                        intent=intent,
                        ...
                        # Child rows such as room/dining_service often inherit their
                        # destination through property_id and therefore have no direct
                        # destination_id. A closed named-entity scope below is stricter
                        # than destination matching and safely admits those children.
                        require_destination_id_match=bool(
                            exhaustive_requested and not named_entity_scope.get("entity_ids")
                        ),
                    )
                    if exhaustive_requested and named_entity_scope.get("names"):
                        candidates = [
                            item for item in candidates
                            if self._document_matches_named_entity_scope(
                                item, named_entity_scope
                            )
                        ]
    ...
    @staticmethod
    def _context_round_robin(
        documents: list[dict[str, Any]],
        *,
        task_aware: bool = False,
    ) -> list[dict[str, Any]]:
        """Interleave branches so one long task/intent cannot hide the rest."""
        groups: dict[str, list[dict[str, Any]]] = {}
        order: list[str] = []
        for item in documents:
            key = str(
                item.get("matched_task_id")
                if task_aware and item.get("matched_task_id")
                else item.get("matched_intent") or "general"
            )
    ...
    def build_context_with_diagnostics(
        ...
        *,
        ...
        for index, item in enumerate(ordered, start=1):
            ...
            block = (
                f"[SOURCE {index}]\n"
                f"task_ids: {', '.join(str(value) for value in (item.get('matched_task_ids') or [item.get('matched_task_id')]) if value) or 'none'}\n"
                f"task_goal: {item.get('matched_task_goal') or ''}\n"
            ...
            for task_id in task_ids:
                selected_tasks[task_id] = selected_tasks.get(task_id, 0) + 1
            entity_key = str(
                metadata.get("entity_id")
                or metadata.get("entity_name")
                or item.get("id")
